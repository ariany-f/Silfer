...
const StockDetails = (props) => {
    ...
    return (
        <MasterLayout>
            <TopProgressBar />
            <HeaderTitle title={getFormattedMessage('stock.report.details.title')} to='/app/user/report/report-stock'/>
                        ...
                        {stockWarehouse && stockWarehouse.map((warehouse, index) => {
                            return (
                                <tr key={index}>
                                    ...
                                    <td>{(warehouse.quantity).toFixed(2)} {warehouse?.product_unit_name?.name}</td>
                                </tr>
                            )
                        })}
